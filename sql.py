...
def pulltrades(date):
    ...
    sql = 'SELECT*  FROM trades where "dateTime" > {} order by "dateTime" desc'.format(date)
...
def pullAllF2Position(date):
    ...
    cnxn = Connection('LIVE-BOSQL1','FuturesII')
    sql = """select DISTINCT productId, prompt, optionTypeId, strike,  (buyLots - sellLots) as quanitity from DBO.OpenPositionCOB 
                where positionHolderId in ('90601', '90602', '90603', '90604', '90605') and cobDate = '{}' and (buyLots - sellLots) <>0 and productId NOT IN ('TCAO', 'TADO', 'TZSO', 'TNDO')""".format(date)
    df = pd.read_sql(sql, cnxn)
    cnxn.close()   
    return df
