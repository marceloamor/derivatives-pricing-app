...
from parts import topMenu, expiryProcess, timeStamp, updateRedisDelta, updateRedisPos, updateRedisTrade, updatePos, onLoadProduct 
...
options = dbc.Row([  
         ...
         dbc.Col([ html.Button('Run', id='run', style={'background':'#F1C40F'})    ], width = 3),
         dbc.Col([html.Button('Expiry', id='expiry', style={'background':'#F1C40F'})   ], width = 3), 
         dbc.Col([html.Button('Select All', id='all-button', style={'background':'#F1C40F'})   ], width = 3) 
        ])
...
def initialise_callbacks(app):
    ...
    def update_expiry(click, ref, product):
        if click:
            ...
            return  dtable.DataTable(id='expiryTable', columns = columns, data = dict,          
                                row_selectable=True,
                                editable=True,
                                    )

    #send trade to system
    @app.callback(Output('trade-div', 'children'),
        ...
        if indices:
            for i in indices:
                ...
                if rows[i]['instrument'][3] =='O':
                    ...
                    trade = TradeClass(0, timestamp, product, strike, CoP, prompt, price, qty, counterparty, '', user, 'Expiry Process')
                    ...
                    trade = TradeClass(0, timestamp, product, None, None, prompt, price, qty, counterparty, '', user, 'Expiry Process')
