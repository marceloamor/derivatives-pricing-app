    ...
    [
        ...
        dbc.FormGroup(
            [
                ...
                dbc.Input(type="text", id= 'codename', placeholder="For dropdown"),
                dbc.Label("Firm", className="mr-2"),
                dbc.Input(type="text", id= 'firm', placeholder="Firm name"),
                dbc.Label("Contact", className="mr-2"),
                dbc.Input(type="text", id= 'contact', placeholder="Main contact"),
                dbc.Label("E-Mail", className="mr-2"),
                dbc.Input(type="text", id= 'contact_email', placeholder="Main Email"), 
                dbc.Label("Clearing E-Mail", className="mr-2"),
                dbc.Input(type="text", id= 'clearing_email', placeholder="Clearing Email"), 
                dbc.Label("Clearing Code", className="mr-2"),
                dbc.Input(type="text", id= 'clearing_code', placeholder="Code for crossing"),                                                   
                dbc.Label("Cat Member", className="mr-2"),
                dbc.Input(type="text", id= 'cat_member', placeholder="Code for crossing"),  
                dbc.Label("Agreed Rate", className="mr-2"),
                dbc.Input(type="numeric", id= 'bro_rate', placeholder=0.25),                               
            ],
            className="mr-3",
        )
    ],
...
layout = html.Div([
       ...
       dbc.Row([
           dbc.Col([
            dtable.DataTable(id = 'broker_table',                              
                            data=broker_data()[0],
