...
def buildTradeEntry(
    product, strike, CoP, prompt, price, qty, counterparty, comment, submit
):
    trade_entry = html.Div(
        [
            html.Div(
                [dcc.Dropdown(id=product, value="LAD", options=onLoadProduct())],
                className="one columns",
            ),
            html.Div(
                [dcc.Dropdown(id=strike, placeholder="Strike")], className="one columns"
            ),
            html.Div(
                [
                    dcc.Dropdown(
            ...
            html.Div(
                [dcc.Input(id=price, placeholder="Enter a price..", type="text")],
                className="one columns",
            ),
            html.Div(
                [
                    dcc.Input(
                        id=qty, placeholder="qty", type="numeric", inputmode="numeric"
                    )
                ],
                className="one columns",
            ),
            html.Div(
                [dcc.Input(id=counterparty, value=" ", type="text")],
                className="two columns",
            ),
            html.Div(
                [dcc.Input(id=comment, value=" ", type="text")], className="two columns"
            ),
...
options = dbc.Col(
    [html.Button("Rec", id="rec", style={"background": "#F1C40F"})], width=2
)
