...
def PostGresEngine():
    engine = create_engine('postgres://jtadtrdlaubnst:2916130e4ae2957b033115855a31fe3ec126d217fdeab4121d104154909a2430@ec2-54-247-113-90.eu-west-1.compute.amazonaws.com:5432/ddb17pbfj9e7cv')
    return engine
...
def pullAllF2Position(date):
    cnxn = PostGresEngine()
    sql = """select productId, prompt, optionTypeId, strike,  (buyLots - sellLots) as quanitity from DBO.OpenPositionCOB 
                where positionHolderId in ('90601', '90602', '90603', '90604', '90605') and cobDate = '{}' and (buyLots - sellLots) <>0""".format(date)
    df = pd.read_sql_query(sql, cnxn) 
    return df

def deletePositions(date, product):
    cnxn = PostGresEngine()
    cursor.execute( """delete FROM positions where left(instrument,3)='{}'""".format(product))
    cursor.commit()
...
def storeTradeSend(trade, response):
    ...
    #data to send 
    data = (trade.timestamp, trade.product, trade.strike, trade.cop, trade.price, trade.qty, trade.countPart, trade.user, trade.venue, status, message)

    #send message to SQL
    cursor = Cursor('Sucden-sql-soft','LME' )
    cursor.execute('insert into routeStatus values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);', data)
    cursor.commit()
