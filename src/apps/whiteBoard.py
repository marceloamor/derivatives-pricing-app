...
messageEntry = html.Div(
    [
        html.Div(
            [
                dcc.Textarea(
                    id="textMessage", placeholder="Message...", style={"width": "100%"}
                )
            ],
            className="row",
        ),
        html.Div(
            [
                html.Div(
                    [html.Button("Send", id="send", n_clicks_timestamp="0")],
...
def initialise_callbacks(app):
    # pull logs
    @app.callback(Output("textLines", "rows"), [Input("wblive-update", "n_intervals")])
