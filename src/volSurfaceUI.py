...
def draw_param_graphTracesold(results, param):
    ...
    layout = go.Layout(
        ...
        showlegend=True,
        xaxis={
            "range": [min(strikes), max(strikes)],
            "fixedrange": True,
            "title": "Strike",
        },
        yaxis={
            "range": [
                min(params) - (0.10 * min(params)),
                max(params) + (0.10 * max(params)),
            ],
            "fixedrange": True,
            "title": "Vol",
        },
...
def draw_price_graphTraces(tickdata):
    ...
    layout.append(
        go.Layout(
            xaxis={"range": [min(time), max(time)], "title": "TimeStamp"},
            yaxis={
...
productDropdown = dbc.Col(
    [
        ...
        dbc.Row([dbc.Col([dcc.Dropdown(id="month-selector")])]),
        dbc.Row([dbc.Col([html.Div(id="product")])]),
    ],
    width=2,
)

infoCurrentPrice = dbc.Col(
    [
        dbc.Row(
            [
...
columns = [
    {"name": "Call Theo", "id": "calc_price_call"},
            ...
            [
                dtable.DataTable(
                    ...
                    style_data_conditional=[
                        ...
                        {
                            "if": {"column_id": "strike"},
                            "background-color": "rgb(171, 190, 249)",
                        },
                        {
                            "if": {
...
paramInputs = html.Div(
    [
        ...
        dbc.Row(
            [
                dbc.Col([html.Button("Submit", id="button")], width=2),
                dbc.Col(
                    [
...
layout = html.Div(
    [
        ...
        dbc.Row([productDropdown, infoCurrentPrice]),
        # graphs,
        paramInputs,
...
def initialise_callbacks(app):
    ...
    @app.callback(
        [Output("month-selector", "options"), Output("month-selector", "value")],
    ...
    @app.callback(
        ...
        [Input("product-selector", "value"), Input("month-selector", "value")],
    )
            ...
            if intermediate_data != None and type(intermediate_data) != int:
                ...
                if "calc_price" in dff.columns:
                    ...
                    combinded = dff.loc[dff.cop == "c"][
                        ["strike", "instrument", "delta", "calc_price", "fullDelta"]
                    ].merge(
                        dff.loc[dff.cop == "p"],
                        how="left",
                        on="strike",
                    ...
                    if combine == "single":
                        return [
                            combinded.round(3).to_dict("records"),
                        ...
                        dff = (
                            dff.groupby(
                                pd.cut(
                                    dff["Cdelta"], bucketRange, labels=bucketRange[1:]
                                )
                            )[
                                [
                                    "Ppos",
    ...
    @app.callback(
        ...
        [Input("forward", "children"), Input("cref", "children")],
    )
    ...
    @app.callback(
        ...
        + [
            State("{}".format(i), "value")
            for i in ["spread", "vola", "skew", "calls", "puts", "cmax", "pmax", "ref"]
        ],
    )
    def blankout(
        product,
    ...
    # update ringtime
    @app.callback(Output("ringVs", "children"), [Input("live-update", "n_intervals")])
